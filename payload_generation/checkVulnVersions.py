...
def generateCVEJson(infos: list, output_file: str = "VersionInfo_CVE.json"):
    cve_api = "https://services.nvd.nist.gov/rest/json/cves/2.0?virtualMatchString=cpe:2.3:*:"
    final_output = {}

    for info in infos:
        info = info.strip()
        vendor, product, version = info.split(":")
        product_string = f"{vendor}:{product}:{version}"
        ...
        for vuln in json_response.get("vulnerabilities", []):
